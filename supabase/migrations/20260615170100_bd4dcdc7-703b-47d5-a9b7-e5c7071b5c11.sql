CREATE OR REPLACE FUNCTION public.is_recent_support_submission_draft(_submission_id text)
RETURNS boolean
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.is_recent_support_submission_draft(text) TO anon, authenticated, service_role;
...
CREATE POLICY "Upload only against active submission draft" ON storage.objects
  ...
  WITH CHECK (
    ...
    AND public.is_recent_support_submission_draft(split_part(name, '/', 2))
  );